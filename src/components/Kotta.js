  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        ...
        const productDetailsResponse = await axios.get(`https://kharthikasarees.s3.eu-north-1.amazonaws.com/KottaCotton/ProductDetails.json`);
      ...
      <div className="thumbnail-grid">
        {products.map((product) => (
          <div key={product.id} className="thumbnail-card">
            <Link to={`/KottaCotton/${product.id}`} style={{ textDecoration: 'none' }}>
