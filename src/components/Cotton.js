...
const Cotton = () => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        const productDetailsResponse = await axios.get(`https://kharthikasarees.s3.eu-north-1.amazonaws.com/cotton/ProductDetails.json`);
      ...
      <div className="thumbnail-grid">
        {products.map((product) => (
          <div key={product.id} className="thumbnail-card">
            <Link to={`/cotton/${product.id}`} style={{ textDecoration: 'none' }}>
